...
export default function VideoPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Video Studio"
        description="HyperFrames-powered generation — HTML to MP4 with animations, rain, particles & zen effects."
      />
        ...
        <Card className="p-5 space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-bg-hover border border-border rounded-[var(--radius)] px-4 py-3 text-sm text-text-primary placeholder:text-text-muted outline-none focus:border-accent/50 resize-none"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-bg-hover border border-border rounded-[var(--radius)] px-4 py-3 text-sm text-text-primary placeholder:text-text-muted outline-none focus:border-accent/50"
            />
          </div>
          ...
          <div>
            ...
            {bgImages.length === 0 ? (
              <button
                type="button"
                onClick={loadBgImages}
                className="w-full p-3 rounded-[var(--radius)] border border-dashed border-border text-xs text-text-muted hover:border-accent/40 hover:text-text-secondary transition-all"
              >
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 max-w-full">
                <button
                  ...
                  className={`flex-shrink-0 w-14 h-14 rounded-[var(--radius)] border-2 flex items-center justify-center transition-all ${
                ...
                {bgImages.slice(0, 8).map((img) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-14 h-14 rounded-[var(--radius)] border-2 overflow-hidden transition-all ${
          ...
          <Button
            ...
            fullWidth
            size="lg"
            icon={status === "rendering" ? <Loader2 size={18} className="animate-spin" /> : <Wand2 size={18} />}
          >
            {status === "rendering" ? "Rendering..." : "Generate Video"}
          </Button>

          {error && (
            <p className="text-xs text-error bg-error/5 p-3 rounded-[var(--radius)]">{error}</p>
          )}
        </Card>

        <Card lit className="p-5 space-y-4 flex flex-col min-h-[420px]">
          ...
          {status === "done" && result && (
            <div className="flex-1 space-y-4">
              ...
              <div className="bg-black rounded-[var(--radius-lg)] overflow-hidden flex items-center justify-center aspect-[9/16] max-h-[400px]">
                <video src={result.video} controls autoPlay loop muted className="max-w-full max-h-full" />
              </div>
              ...
              {result.video && (
                <Button href={result.video} download size="sm" icon={<Download size={14} />}>
                  Download MP4
                </Button>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {templates.map((t) => (
            <button
              ...
              className={`p-4 rounded-[var(--radius)] border text-left transition-all ${
                template === t.key
